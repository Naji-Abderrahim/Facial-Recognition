...
class Root:
    ...
    if sys.platform.startswith('win'):
        root.iconbitmap('/icons/img_par_defaut.png')
    else:
        logo = PhotoImage(file='icons/img_par_defaut.png')
        root.tk.call('wm', 'iconphoto', root, logo)

    image_background = PIL.ImageTk.PhotoImage(PIL.Image.open("images/image_background.png"))
    video_background = PIL.ImageTk.PhotoImage(PIL.Image.open("images/video_background.png"))
    cross = PIL.ImageTk.PhotoImage(PIL.Image.open("images/cross.png"))
    refresh = PIL.ImageTk.PhotoImage(PIL.Image.open('images/refresh.png'))
        ...
        if capt.isOpened():
            ...
            if message < 30:
                if ret:
                    ...
                    Information.info.config(text="number of faces : " + str(faces) + "\nnum of similar faces to ref : "
                                                 + str(similarfaces) +
                                                 "\nnum of different face to ref : " + str(nonsimilarfaces))
                self.video_frame.after(
                        10,
                        lambda: VideoProcess.playVideo(
                            self=VideoProcess.__new__(VideoAnime),
                            capt=capt
                            )
                        )
            # else:
            #     capt.release()
            #     Information.info.config(text="the face has been founded !\n")
            #     photo = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
            #     self.can.create_image(230, 137, image=photo, anchor=CENTER)
...
class LiveCamera(RefreshButton):
    ...
    def getCameras(self: RefreshButton):
        global index_of_record
        if sys.platform.startswith('win'):
            name = VideoInterface.getCameraWin()
            index = 0
            self.live_camera_capture_chose.config(values=name)
            self.live_camera_capture_chose.current(0)
            index_of_record = index
        else:
            name, index = VideoInterface.getCameraLinux()
            self.live_camera_capture_chose.config(values=name)
            self.live_camera_capture_chose.current(0)
            index_of_record = index
